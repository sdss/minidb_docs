...
import click
tbd = "TBD in minidb_docs (as of 09/27/2022)"


def summary_from_txt(summary_file = None, table_name = None, release = None, product = None):
    ...
    if summary_file:
        ...
        if not summary_file.exists():
            raise FileExistsError(f"{summary_file} does not exist.")
        ...
        for term, replace in {"%s_" % release: "%s_" % product}.items():
            if term in header:
                header = header.replace(term, replace)

        data['header'] = header = header.strip()
        if not header: data['header'] = tbd
        data['description'] = description_list = [item.strip() for item in description_list if item.strip() != ""]
        ...
        for column in column_list:
            ...
            if column_description:
                for term, replace in {"%s_" % release: "%s_" % product}.items():
                    if term in column_description: column_description = column_description.replace(term, replace)
                
            data['columns'][column_name] = {
                'description': column_description.strip() if column_description else tbd,
                'units': column_units.strip() if column_units else ""
            }
